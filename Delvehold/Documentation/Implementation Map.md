...
- Delvehold owns the semantic [[Control Flow Map|architecture and control-flow graph]] derived from its current source. Norn renders that typed projection and owns presentation behavior only. Selection and inspection are non-mutating; gameplay actions still cross the existing command-and-receipt boundary.
