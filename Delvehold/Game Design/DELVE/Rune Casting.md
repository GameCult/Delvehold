...
Rune casting is live construction in the sequential source language governed by shared [[Game Design/Shared World/Runic Semantics|runic semantics]]. A caster enters spellcasting mode, adds strokes and casting runes to one append-only canvas, and presses the same mode control again to finalize and exit. Finalization, not rune recognition, commits the sequence for authoritative lowering and execution.
...
- the rune-library control opens a radial projection of every casting rune the character has learned.

The learned casting-rune set is world state. Opening the library does not grant knowledge or create a second casting mode.

## Direct and guided construction

Direct freehand casting records the cursor trajectory while the mana trigger is held. The detector matches completed strokes against familiar casting runes and appends recognized tokens to the active sequence.

Guided library casting is available to every player on every cast. Selecting a learned casting rune places a virtual cursor at the first canonical stroke start. Pointer movement with a positive projection onto the current stroke tangent advances that cursor; orthogonal and backward movement stall without erasing progress. Advancement has a maximum speed and is scaled by directional alignment raised to a tunable exponent. These are balance handles for keeping guided casting competitive while preserving alignment and rhythm as mastery.
...
Both paths use the same casting-rune geometry, learned token identity, active sequence, lowering path, and world result. Guided casting removes recall and lateral precision demands while retaining rune order, direction changes, pen-up transitions, trigger cadence, and casting pressure. Players may switch between guided and freehand construction rune by rune within one spell.

The casting grammar is deliberately compact. Frequently used graph motifs can be learned as compound casting runes. The common `Attach → Creation → Throw` motif has one compound gesture whose expansion attaches the current construct to a newly allocated dynamic entity and throws that carrier. Viewing an expansion in a spellbook does not silently grant the corresponding enchanting knowledge.
...
Exiting spellcasting mode finalizes the canvas. The host recognizes the casting sequence, expands compound runes, lowers it into the semantic effect graph, and validates the outer terminator. A valid graph executes. A null-terminated sequence cancels safely. Failed recognition, invalid sequential grammar, failed compound expansion, or a corrupt, incomplete, or unterminated lowered graph becomes wild magic.
...
The spellbook records finalized attempts, including successful, apparently useless, cancelled, and wild results. Its baseline entry contains the submitted stroke evidence, recognized casting sequence, compound expansions, source map, authoritative semantic graph, final status, ordinary visible mana change, and consequences the character observed. The graph uses a Sugiyama layered projection beside an equivalent linear casting-rune list and relational text outline.
